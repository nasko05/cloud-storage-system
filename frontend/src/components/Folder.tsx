...
export function FolderItem({ folder }: FolderItemProps): React.ReactElement {
  return (
    <Stack direction="row" alignItems="center" spacing={1.5} minWidth={0}>
      <FolderRoundedIcon sx={{ fontSize: 28, color: '#818CF8', flexShrink: 0 }} />
...
export function FolderIcon({ size = 48, color = '#818CF8' }: FolderIconProps): React.ReactElement {
  ...
  return (
    <Stack direction="row" spacing={0.5}>
      <Tooltip title="Download as ZIP">
        <IconButton size="small" sx={{ color: 'text.secondary', '&:hover': { color: 'primary.main' } }} onClick={() => onDownload(folder)}>
          <DownloadRoundedIcon fontSize="small" />
        </IconButton>
      </Tooltip>
      {onDelete && (
        <Tooltip title="Delete folder">
          <IconButton size="small" sx={{ color: 'text.secondary', '&:hover': { color: 'error.main' } }} onClick={() => onDelete(folder)}>
